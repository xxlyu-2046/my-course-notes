...
• 按照正向传播的公式，如果方向传播的时候对其求导数会导致权重为0，因此反向传播的时候相当于一个直接估算器，得到：
...
Remove all redundant nodes without changing the graph structure. <u>在不改变图形结构的情况下删除 所有冗余节点。</u>
...
| Duplicate Reshape Eliminate                    | RS消除         | 连续 Reshape 只需要保留最后一个 Reshape                                                                            |
| Duplicated Cast Eliminate                      | RS消除         | 连续的内存排布转换或者数据转换，只需要保留最后一个                                                                 |
...
**公共子图：**最大公共子图问题是给定两个图，要求去掉一些点后，两个图都得到一个节点数 至少为b的子图，且两个子图完全相同。
